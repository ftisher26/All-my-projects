...
mainForm.addEventListener("submit", async (event) => {
	...
	const privacyCheck = document.getElementById("checkbox");
...
if (userName.trim() === "" && lastName.trim() === "" && userEmail.trim() === "" 
	&& message.trim() === "" && !privacyCheck.checked) {
	...
	privacyCheck.classList.add("invalid");
	esValido = false;
	event.preventDefault();
	return; 
} else if (userName.trim() === "" || lastName.trim() === "" || userEmail.trim() === "" || 
		message.trim() === "" || !privacyCheck.checked) {
		errorPopup.classList.add("is-visible");
		errorBar.classList.add("is-visible");
		setTimeout(() => {
